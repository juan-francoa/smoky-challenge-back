...
const controller = {
  create: async (req, res) => {
    //método para crear un Equipment
    try {
  ...
  read: async (req, res) => {
    //método para leer/obtener todos los Equipment
    let { query } = req;
      ...
      for (let name in query) {
        ...
        if (name === "category") {
          if (query.category.length) {
            array = array.filter((e) =>
              query.category.toLowerCase().includes(e.category.toLowerCase())
            );
          }
        }
      }
  ...
  update: async (req, res) => {
    ...
    try {
      let equipment = await Equipment.findOneAndUpdate({ _id: id }, req.body, {
        new: true,
      });
      if (equipment) {
        res.status(200).json({
