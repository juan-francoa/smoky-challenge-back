...
const controller = {
  addShop: async (req, res) => {
    ...
    try {
      ...
      if (user) {
        let num = user.products.filter(
          (e) => Object.keys(req.body.products)[0] == Object.keys(e)[0]
        );
        console.log(num, 1);
        if (num.length > 0) {
          num = num.map(
            (e) =>
              ...
              Number(req.body.products[Object.keys(e)[0]])
          );
          ...
          let nuevo = await Shopping.findOneAndUpdate(
            { userId: id },
            { products: obj },
            { new: true }
          );
